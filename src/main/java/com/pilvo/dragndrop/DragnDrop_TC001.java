...
public class DragnDrop_TC001 {
	...
	public void check_action() throws InterruptedException{
		...
		actions.clickAndHold(source).moveToElement(destination, -100, 170).release().build().perform();;
		...
		actions.clickAndHold(source).moveToElement(destination, 290, 120).release().build().perform();
		...
		/*
		 * act.moveToElement(source) .pause(1000) .clickAndHold(source) .pause(1000)
		 * .moveByOffset(1, 0) .moveToElement(destination) .moveByOffset(1, 0)
